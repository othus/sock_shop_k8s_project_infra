...
module "bastion" {
  ...
  subnets = data.aws_subnet.pubsub1
  key_name = module.SG_keypair.project_key
  private-key = module.SG_keypair.keypair
  bastion-sg = module.SG_keypair.bastion-ansible-sg
}
...
module "master_node" {
  ...
  master_worker_sg = module.SG_keypair.master_worker_sg
  keypair_name = module.SG_keypair.keypair
...
module "ansible" {
  ...
  private_key = module.SG_keypair.keypair
  key_name = module.SG_keypair.key_name
...
module "route53_ssl" {
  ...
  grafana_dns_name = module.monitoring_lb.grafana-lb
